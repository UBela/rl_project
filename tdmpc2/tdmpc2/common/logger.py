...
def print_run(cfg):
	...
	# Debug: Print the type of cfg.obs_shape
	print("DEBUG: Type of cfg.obs_shape:", type(cfg.obs_shape))
	#print(cfg.obs_shape.values())
	#print(cfg)
	#observations  = ", ".join([str(v) for v in cfg.obs_shape.values()])
	observations = ", ".join([str(v) for v in cfg.obs_shape.values()]) if isinstance(cfg.obs_shape, dict) else str(cfg.obs_shape)
	kvs = [
		("task", cfg.task_title),
...
class Logger:
	...
	def __init__(self, cfg):
		...
		wandb.login(key="1c19e048ca5da79c677efbf93e742cd1114c0d5b")
