...
from envs.utils.replay_buffer import ReplayBuffer, PriorityReplayBuffer
...
class HockeyEnv(gym.Env, EzPickle):
  ...
  def reset(self, one_starting=None, mode=None, seed=None, options=None):
    #print("Hier sind wir im großen reset")
    self._destroy()
    self.seed(seed)
    ...
    if self.mode != Mode.NORMAL:
      ...
      #print("Hier wurde der Player auf dumm gestellt.")
  ...
  def step(self, action):
    #print("Hier sind wir im großen step")
...
# class HockeyEnvWrapper(gym.Wrapper):
...
#     self.current_opponent = BasicOpponent(weak=True)  # Start with weak opponent
#     self.opponents = [BasicOpponent(weak=True), BasicOpponent(weak=False)]
#     self.self_play_start = self.cfg.get("self_play_start", 10)
#     self.toggle_basic_opponents = self.cfg.get("toggle_basic_opponents", 60)
#     self.self_play_intervall = self.cfg.get("self_play_intervall", 50)
#     self.use_self_play = False  # Start without self-play mode
#     
# 
#   def _toggle_basic_agent(self):
#     """Switch between weak and strong opponents or self-play mode."""
#     if self.current_episode <= self.self_play_start and self.current_episode % self.toggle_basic_opponents == 0:
#       if isinstance(self.current_opponent, BasicOpponent):
#         if self.current_opponent.weak:
#           print("Switching to strong opponent...")
#           self.current_opponent = BasicOpponent(weak=False)
#         else:
#           print("Switching to weak opponent...")
#           self.current_opponent = BasicOpponent(weak=True)
#   
#   def _add_self_play_agent(self):
#     if self.current_episode >= self.self_play_start:# and self.current_episode % self.self_play_intervall == 0:
#       print("Saving current agent...")
#       self.logger.save_agent(agent, identifier="selfplay")
#       #Logger.save_agent(self, identifier='selfplay')
#       print("loading current...")
#       agent = TDMPC2(self.cfg)
#       agent.load("/mnt/qb/work/geiger/gwb215/test/tdmpc2/tdmpc2/logs/hockey-NORMAL/1/default/models/selfplay.pt")
#       print("Adding agent to self-play opponents...")
#       self.opponents.append(copy.deepcopy(agent))
#       print("done!")
# 
#   def reset(self, **kwargs):
#     self.current_episode += 1
#     obs, info = super().reset(**kwargs)  # Unpack tuple
#       
#     self.env.step(np.zeros(self.env.action_space.shape))  # To get initial state
#     return obs  # Return only obs to avoid tuple issues
# 
#   def step(self, action):
#     # Perform environment step
#     obs, reward, terminated, truncated, info = self.env.step(action.copy())  # Get five outputs
#     done = terminated or truncated  # Merge terminated & truncated into done
# 
#     if done:  # When episode ends
#       self._toggle_basic_agent()
#       self._add_self_play_agent()
# 
#     return obs, reward, done, info  # Return only four values for compatibility
#     
#   def render(self, mode="rgb_array"):
#     """Override render to always return RGB arrays for recording"""
#     frame = self.env.render(mode="rgb_array")  # Force 'rgb_array' mode
#     if frame is not None:
#       return np.array(frame)  # Ensure it's a NumPy array
#     else:
#       print("Warning: env.render(mode='rgb_array') returned None!")
#       return None
# 
# 
# def make_env(cfg):
#   """Creates and returns the HockeyEnv environment with configuration."""
#   from envs.wrappers.timeout import Timeout  
#   # Check if the task name starts with 'hockey-'
#   if not cfg.task.startswith("hockey-"):
#     raise ValueError(f"Unknown task: {cfg.task}. Expected a task starting with 'hockey-'")
# 
#   keep_mode = cfg.get("keep_mode", True)
#   mode = cfg.task.split("hockey-")[-1]  # Extract the mode from task name
#   verbose = cfg.get("verbose", False)
#   logger = Logger(cfg)
#   
#   env = HockeyEnv(keep_mode=keep_mode, mode=mode, verbose=verbose)
#   env = HockeyEnvWrapper(env, cfg, logger)
#   env = Timeout(env, max_episode_steps=cfg.get("max_episode_steps", 1000))  # Timeout wrapper
# 
#   return env
...
class HockeyEnvWrapper(gym.Wrapper):
  def __init__(self, env, cfg, agent):
    super().__init__(env)
    self.env = env
    self.cfg = cfg
    self.agent = agent  # Store the agent
    self.current_episode = 0
    self.self_play_start = cfg.get("self_play_start", 500)
    self.self_play_intervall = cfg.get("self_play_interval", 20)
    self.toggle_basic_opponents = cfg.get("toggle_basic_opponents", 20)
    self.past_agents = [BasicOpponent(weak=False), BasicOpponent(weak=False)]  
    self.env.opponent_ai = self.past_agents[0]
    #self.env.player2 = self.env.create_opponent_body()

  def _toggle_basic_agent(self):
    """Toggles between weak and strong BasicOpponent before self-play starts."""
    if self.current_episode <= self.self_play_start and self.current_episode % self.toggle_basic_opponents == 0:
      if isinstance(self.env.opponent_ai, BasicOpponent):
        if self.env.opponent_ai.weak:
          print("Switching to strong opponent...")
          self.env.opponent_ai = BasicOpponent(weak=False)
        else:
          print("Switching to weak opponent...")
          self.env.opponent_ai = BasicOpponent(weak=False)

  def _switch_to_self_play(self):
    """Replaces the opponent with a past version of the trained agent after self_play_start."""
    if self.current_episode >= self.self_play_start and self.past_agents:
      #self.current_opponent = random.choice(self.past_agents)
      self.env.player2 = self.past_agents[-1]
      print(f"Switched to self-play using a past agent (Episode {self.current_episode})")

  def reset(self, **kwargs):
    self.current_episode += 1
    obs, info = super().reset(**kwargs)
    self.env.step(np.zeros(self.env.action_space.shape)) #init env

    #if self.current_episode < self.self_play_start:
    #  self._toggle_basic_agent()
    #else:
  ...
  def step(self, action):
    ...
    #print(f"Opponent obs at step {self.current_episode}: {obs_opponent}")
    obs_opponent_tensor = torch.from_numpy(obs_opponent).float() # Ensure it's float type
    opponent_action = self.env.opponent_ai.act(obs_opponent_tensor)
    #combine the actions.
    ...
    if done:
        if self.current_episode >= self.self_play_start:
            # Save a past version of the trained agent periodically
            if self.current_episode % self.self_play_intervall == 0:
                self.past_agents.append(copy.deepcopy(self.agent))  # Store the agent
                print(f"Saved a new past agent for self-play (Total: {len(self.past_agents)})")
...
def make_env(cfg, agent):
  ...
  env = HockeyEnv(keep_mode=keep_mode, mode=mode, verbose=verbose)
  env = HockeyEnvWrapper(env, cfg, agent)  # Pass the agent instance
  env = Timeout(env, max_episode_steps=cfg.get("max_episode_steps", 1000)) 
...
#     self.cfg = cfg
#     
#     self.use_self_play = cfg.get("use_self_play", True)
#     self.self_play_start = cfg.get("self_play_start", 500)
#     self.self_play_intervall = cfg.get("self_play_intervall", 50)
#     self.toggle_basic_opponents = cfg.get("toggle_basic_opponents", 60)
# 
#     # Opponent pool: Start with weak and strong opponents
#     self.opponents = [BasicOpponent(weak=True), BasicOpponent(weak=False)]
#     self.current_opponent = BasicOpponent(weak=True)
#     self.current_episode = 0
#     #self.agent = None # Add agent attribute
#     
#   def _select_opponent(self):
#     """Randomly selects an opponent from the pool."""
#     return np.random.choice(self.opponents)
# 
#   def _add_self_play_agent(self, agent):
#     """Adds a trained agent to self-play after a certain number of episodes."""
#     if not self.use_self_play or agent is None:
#       return
# 
#     if self.current_episode >= self.self_play_start and self.current_episode % self.self_play_intervall == 0:
#       print("Adding agent to self-play opponents...")
...
#     self.current_opponent = self._select_opponent()  # Randomly select an opponent
# 
#     return obs  # Return only obs to avoid tuple issues
# 
#   def step(self, action):
#     if not hasattr(self, 'current_opponent') or self.current_opponent is None:
#       self.current_opponent = self._select_opponent()
#     obs, reward, terminated, truncated, info = self.env.step(action.copy())  # Get five outputs
#     done = terminated or truncated  # Merge terminated & truncated into done
#     
#     if done:  # When episode ends
#       self._add_self_play_agent(self.agent)
#       self._toggle_basic_agent()
# 
#     return obs, reward, done, info  # Return only four values for compatibility
#   
...
#     """Override render to always return RGB arrays for recording"""
#     frame = self.env.render(mode="rgb_array")  # Force 'rgb_array' mode
#     # print("Ich bin im render header")
#     if frame is not None:
#       return np.array(frame)  # Ensure it's a NumPy array
#     else:
#       print("Warning: env.render(mode='rgb_array') returned None!")
...
#   #if agent is not None:  # Set the agent if it's provided
#   #
#   # env.set_agent(agent)
#   env = Timeout(env, max_episode_steps=cfg.get("max_episode_steps", 1000))  # Timeout wrapper
# 
...
#########################################
# Wrapper for Prioritized Replay Buffer #
#########################################
...
#     self.env = env
#     self.cfg = cfg
#     self.replay_buffer_type = cfg.get("replay_buffer_type", "basic")  # "basic" or "prioritized"
#     self.replay_buffer_size = cfg.get("replay_buffer_size", 100000)
#     self._create_replay_buffer()
#     
#     self.current_episode = 0  # Keep track of the current episode
# 
#   def _create_replay_buffer(self):
#       if self.replay_buffer_type == "basic":
#           self.replay_buffer = ReplayBuffer(max_size=self.replay_buffer_size)
#       elif self.replay_buffer_type == "prioritized":
#           self.replay_buffer = PriorityReplayBuffer(max_size=self.replay_buffer_size)
#       else:
...
#     return obs
#   
#   def step(self, action):  # Add obs as an argument
#     obs, reward, terminated, truncated, info = self.env.step(action)
#     done = terminated or truncated
#     transition = (obs, action, reward, obs, done)
...
#################################
# Wrapper for weak/strong change
#################################
...
#     self.env = env
#     self.cfg = cfg
#     self.change_opponent_interval = cfg.get("change_opponent_interval", 200)
#     self.num_episodes = cfg.get("num_episodes", 1000)
#     self.opponent_type_stage1 = cfg.get("opponent_type_stage1", "basic")
#     self.opponent_kwargs_stage1 = cfg.get("opponent_kwargs_stage1", {"weak": True})
#     self.opponent_type_stage2 = cfg.get("opponent_type_stage2", "basic")
#     self.opponent_kwargs_stage2 = cfg.get("opponent_kwargs_stage2", {"weak": False})
# 
#     self.opponent_change_conditions_str = cfg.get("opponent_change_conditions", None) # Read condition as a string
#     self.opponent_change_conditions = None
#     self.current_episode = 0
#     self._create_opponent()  # Initialize opponent
#     
#   def _create_opponent(self):
#     opponent_type = self.cfg.get("opponent_type_stage1", "basic")  # Default to stage 1
#     opponent_kwargs = self.cfg.get("opponent_kwargs_stage1", {"weak": True})
#     if opponent_type == "basic":
#       self.opponent = BasicOpponent(**opponent_kwargs)
#     elif opponent_type == "human":
#       self.opponent = HumanOpponent(self.env)  # Pass the base env
#     else:
#       raise ValueError(f"Unknown opponent type: {opponent_type}")
#   
#   def change_opponent(self, opponent_type, opponent_kwargs=None):  # Make it accessible
#     if opponent_kwargs is None:
#       opponent_kwargs = {}
#     if opponent_type == "basic":
#       self.opponent = BasicOpponent(**opponent_kwargs)
#     elif opponent_type == "human":
#       self.opponent = HumanOpponent(self.env)
#     else:
#       raise ValueError(f"Unknown opponent type: {opponent_type}")
#   
#   def reset(self, **kwargs):
...
#     obs, info = super().reset(**kwargs)
#     self.env.step(np.zeros(self.env.action_space.shape))
#     self._maybe_change_opponent()
#     return obs
#   
#   def _maybe_change_opponent(self):
#     if self.opponent_change_conditions is None:
#       if self.current_episode % self.change_opponent_interval == 0:
#         self._perform_opponent_change()
#     else:
#       if self.opponent_change_conditions(self.current_episode, self.env):
#         self._perform_opponent_change()
#   
#   def _perform_opponent_change(self):
#     if self.current_episode < self.num_episodes / 2:
#       self.change_opponent(self.opponent_type_stage1, self.opponent_kwargs_stage1)
#       print("Changing to stronger basic opponent")
#     else:
#       self.change_opponent(self.opponent_type_stage2, self.opponent_kwargs_stage2)
#       print("Changing to stronger basic opponent")
#   
#   def step(self, action):
#     ob2 = self.env.obs_agent_two()  # Get opponent's observation
#     a2 = self.opponent.act(ob2)  # Opponent acts
#     action2 = np.hstack([action, a2])
#     reward = 0
#     for _ in range(2):  # Simulate action repeat
#       obs, r, terminated, truncated, info = self.env.step(action2)  # Use combined action
#       reward += r
#     done = terminated or truncated
#     return obs, reward, done, info
#   
...
#     reward = 0
#     for _ in range(2):  # Simulate action repeat
#         obs, r, terminated, truncated, info = self.env.step(action.copy())  # Get five outputs
#         reward += r
#     done = terminated or truncated  # Merge terminated & truncated into done
#     return obs, reward, done, info  # Return only four values for compatibility
# 
