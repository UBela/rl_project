...
class HockeyEnvWrapper(gym.Wrapper):
    ...
    def step(self, action):
        reward = 0
        for _ in range(2):  # Example: Apply action twice for smoothness
            obs, r, done, truncated, info = self.env.step(action.copy())
            reward += r
            if done or truncated:
                break
        return obs.astype(np.float32), reward, done, truncated, info

# def make_hockey_env(cfg):
...
#     env = HockeyEnvWrapper(env, cfg)  # Apply wrapper if needed
