...
def parse_cfg(cfg: OmegaConf) -> OmegaConf:
	...
	# Convenience
	print("HAHAHAHAHAAHHAHAHAHAHHAHAHAHAHHAHAH")
	print(cfg)
	cfg.work_dir = Path(hydra.utils.get_original_cwd()) / 'logs' / cfg.task / str(cfg.seed) / cfg.exp_name
	#cfg.work_dir = '/mnt/qb/work/geiger/gwb215/test/tdmpc2/tdmpc2/logs' / cfg.task / str(cfg.seed) / cfg.exp_name
	
	cfg.task_title = cfg.task.replace("-", " ").title()
	cfg.bin_size = (cfg.vmax - cfg.vmin) / (cfg.num_bins-1) # Bin size for discrete regression
	# Ensure obs_shape is a tuple
	# If obs_shape is a string, attempt to convert it to a tuple
	try:
		# Use ast.literal_eval to safely evaluate the string as a tuple
		cfg.obs_shape = cfg.obs_shape = {"state": (18,)}
		print("TRYYYYY")
		#cfg.obs_shape = cfg.get("obs_shape")
		#cfg.obs_shape = tuple(cfg.obs_shape)
	except Exception as e:
		print("EXCEPTIONNNNNNN")
		print(f"Error parsing obs_shape: {e}")
