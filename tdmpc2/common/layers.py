...
def enc(cfg, out={}):
	...
	"""
	# cfg.obs_shape = env.observation_space.shape  # or another appropriate source
	# assert cfg.obs_shape != '???', f"obs_shape is not set! Current value: {cfg.obs_shape}"
	print(cfg.obs_shape['state'])
	#cfg.obs_shape = ", ".join([str(v) for v in cfg.obs_shape.values()]) if isinstance(cfg.obs_shape, dict) else str(cfg.obs_shape)
